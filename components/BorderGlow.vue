...
watch(
  ...
  () => {
    ...
    animateValue({ ease: easeInCubic, duration: 1500, end: 50, onUpdate: v => { cursorAngle.value = (angleEnd - angleStart) * (v / 100) + angleStart; } });
    animateValue({ ease: easeOutCubic, delay: 1500, duration: 2250, start: 50, end: 100, onUpdate: v => { cursorAngle.value = (angleEnd - angleStart) * (v / 100) + angleStart; } });
    animateValue({ ease: easeInCubic, delay: 2500, duration: 1500, start: 100, end: 0, onUpdate: v => (edgeProximity.value = v / 100), onEnd: () => (sweepActive.value = false) });
  },
  ...
  >
    ...
    <div
      ...
      :style="{
        ...
        background: [`linear-gradient(${props.backgroundColor} 0 100%) padding-box`, 'linear-gradient(rgb(255 255 255 / 0%) 0% 100%) border-box', ...borderBg].join(', '),
    ...
    >
      <span class="absolute rounded-[inherit]" :style="{ inset: `${props.glowRadius}px`, boxShadow: buildBoxShadow(props.glowColor, props.glowIntensity) }" />
    </span>
    <!-- content -->
